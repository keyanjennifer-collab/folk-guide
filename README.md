...
当前界面以深色交互概念稿为准，以 `COLORO 129-15-00`（屏幕近似 `#262626`）为主视觉，叠加低对比岩面纹理，并包含「今日五色 / 产品商城 / AI国学 / 我的」四个入口。商城包含一款五色线香套装与五款单香，现已具备购物袋、收货地址、服务端计价与库存、微信支付、订单退款以及人工发货闭环。真实收款仍需在生产环境配置微信支付商户资料并显式开启商城开关。

正式小程序 AppID 经项目负责人确认使用 `wx071d4f5e34e12c95`。微信开发者工具应导入本仓库根目录，源码目录为 `miniprogram/`。

- [深色首页视觉参考](preview/dark-concept.html)：固定示例，仅用于视觉验收；小程序首页读取真实当日接口。
- [本次迁移与协作说明](迁移与协作说明.md)：目录、同步范围和待上线事项。
- `preview/index.html` 是保留的历史浏览器预览，不代表当前原生小程序界面。

一个“极薄微信小程序前端 + Python FastAPI 后端”的可运行项目。公开每日五色由确定性历法规则按北京时间自动生成并缓存，不依赖人工运营后台。AI 国学问答支持模型自身知识；审核知识库和联网搜索都由后端开关控制。
...
5. 当前协作项目 AppID 已统一为 `wx071d4f5e34e12c95`，不要再换回旧 AppID。
...
- 正式商品目录、库存锁定与防超卖
- 收货地址、结算、订单取消和整单退款
- 微信支付 API v3 预下单、通知验签解密与支付事件幂等
- 管理员账号登录的订单与库存运营后台、人工录入快递公司和运单号，并记录关键操作审计

## 下一步

1. 增加 `psycopg` 和 Alembic，把 SQLite 开发库迁移为 PostgreSQL 正式方案。
2. 申请域名并部署 HTTPS 后端，配置微信 `request` 合法域名和生产环境强配置校验。
3. 部署数据库迁移后创建首个超级管理员，定期检查后台审计日志并及时停用离职账号。
4. 为多实例部署增加 Redis 限流、微信凭证缓存、日志、监控、告警和数据库备份。
5. 补齐隐私政策、用户协议、AI生成内容说明及小程序备案材料。
6. 完成五色规则专业样本复核、真实微信双账号测试、弱网测试和发布验收。
7. 配置微信支付商户密钥、盘点并录入真实库存，以小额真实订单完成支付、退款和发货验收。

联网搜索、微信订阅消息、古籍向量知识库和完整会员体系仍按产品决定暂缓。商城代码已经具备第一期开售能力，但 `COMMERCE_ENABLED` 在生产环境必须等真实商户资料、库存和小额验收完成后再开启。完整交接状态见 [PROJECT_HANDOFF.md](PROJECT_HANDOFF.md)。
